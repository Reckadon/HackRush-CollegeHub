...
const Events = () => {
  ...
  const isClubCoordinator = userRoles.some((role) =>
    ["blithchron", "amalthea", "metis", "metis-coordinator"].includes(role)
  );
  ...
  useEffect(() => {
    if (selectedFilter === "all") {
      ...
      setFilteredEvents(
        events.filter(
          (event) => event.filters && event.filters.includes(selectedFilter)
        )
      );
    }
  ...
  const handleAddEvent = async (eventData) => {
    ...
    try {
      ...
      alert(
        isAdmin ? "Event added successfully!" : "Event submitted for approval!"
      );
    } catch (error) {
